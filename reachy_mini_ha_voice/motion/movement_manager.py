...
class MovementManager:
    ...
    def __init__(self, reachy_mini: "ReachyMini"):
        ...
        # SDK v1.9 face tracking is a private offset layer. It is never
        # published as a Home Assistant robot control and only runs while the
        # voice interaction is listening or thinking.
        self._face_tracking_enabled = bool(Config.vision.face_tracking_enabled)
        self._face_tracking_active = False
        self._face_tracking_supported = callable(getattr(self.robot, "start_head_tracking", None))
    ...
    def set_face_tracking_enabled(self, enabled: bool) -> None:
        """Enable private SDK face tracking for active voice phases only."""
        self._enqueue_command("set_face_tracking_enabled", bool(enabled), "set_face_tracking_enabled")
    ...
    def _start_emotion_move(self, emotion_name: str) -> None:
        ...
        """
        self._set_face_tracking_active(False)
    ...
    def _set_face_tracking_active(self, active: bool) -> None:
        """Hand head tracking to the SDK without creating a competing loop."""
        if not self._face_tracking_supported or self._face_tracking_active == active:
            return
        try:
            if active:
                self.robot.start_head_tracking(weight=1.0)
            else:
                self.robot.stop_head_tracking()
            self._face_tracking_active = active
            logger.info("SDK face tracking %s", "enabled" if active else "disabled")
        except Exception as exc:
            logger.warning("SDK face-tracking handoff failed: %s", exc)
            self._face_tracking_supported = False

    def _sync_face_tracking(self) -> None:
        desired = self._face_tracking_enabled and self.state.robot_state in {
            RobotState.LISTENING,
            RobotState.THINKING,
        }
        self._set_face_tracking_active(desired)
    ...
    def _update_animation(self, dt: float) -> None:
        ...
        idle_queue_action_active = (
            self.state.robot_state == RobotState.IDLE
            and (idle_transition_action_active or self.state.look_around_in_progress)
            and (
                (
