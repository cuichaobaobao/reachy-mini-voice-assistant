...
def _env_bool(key: str, default: bool) -> bool:
    """Get a boolean from a conventional environment value."""
...
@dataclass
class VisionConfig:
    """Optional on-robot vision feedback configuration.

    Vision remains private to the voice app. The official Home Assistant
    integration owns camera publication and manual robot controls.
    """

    face_tracking_enabled: bool = False
    jpeg_enabled: bool = False
    jpeg_refresh_hz: float = 5.0
...
class Config:
    ...
    def load_from_env(cls) -> None:
        ...
        # Vision
        cls.vision.face_tracking_enabled = _env_bool(
            "REACHY_VISION_FACE_TRACKING_ENABLED", cls.vision.face_tracking_enabled
        )
        cls.vision.jpeg_enabled = _env_bool("REACHY_VISION_JPEG_ENABLED", cls.vision.jpeg_enabled)
        cls.vision.jpeg_refresh_hz = _env_float("REACHY_VISION_JPEG_REFRESH_HZ", cls.vision.jpeg_refresh_hz)
    ...
    def _apply_overrides(cls, data: dict) -> None:
        ...
        if "vision" in data:
            for key, value in data["vision"].items():
                if hasattr(cls.vision, key):
                    setattr(cls.vision, key, value)
    ...
    def to_dict(cls) -> dict:
        ...
        return {
            ...
            "vision": {
                "face_tracking_enabled": cls.vision.face_tracking_enabled,
                "jpeg_enabled": cls.vision.jpeg_enabled,
                "jpeg_refresh_hz": cls.vision.jpeg_refresh_hz,
            },
            "robot_state": {
